...
```{r}
library(cowplot); library(magrittr); library(DMwR);
library(tidyverse); library(caret); library(here)
set.seed(96)
data_path <- "data/soc_ref/ground_truth/02_processed"
...
10-fold cross validation. K-fold cross validation is... [todo: fill in a little background about the arguments to this function]
...
```{r}
gbmGrid <-  expand.grid(interaction.depth = c(5, 9, 14), 
...
```{r}
d_features_preds <- d_features %>% filter(success == 1) 
```
