...
d %>% 
  filter(success == 1, !(gold_code %in% c("away", NA))) %>% 
  filter(gold_code %in% c("left", "right")) %>% 
...
d %>% 
  filter(success == 1, !(gold_code %in% c("away", NA)),
         gold_code %in% c('left', 'right')) %>% 
  plot_ly(data = ., x = ~gaze_0_x, y = ~yaw, z = ~pitch, color = ~gold_code) %>%
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'x_coordinate'),
...
```{r}
d_model <- d %>% 
  filter(success == 1, gold_code %in% c('left', 'right')) %>% 
  mutate(gold_num = ifelse(gold_code == 'left', 1, 0))
```
...
```{r}
m2 <- glm(gold_num ~ gaze_0_x * gaze_0_y * yaw * pitch, data = d_model)
summary(m2)
```
